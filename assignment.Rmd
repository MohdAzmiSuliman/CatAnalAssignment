...
```{r, warning=F}
levhba1c <- c("Target c (>7.0)", "Target b (6.6 - 7.0)", "Target a (<= 6.5)")
anads1 <- anads0 %>%
    mutate(HbA1C_T = cut(HbA1c, breaks = c(0, 6.5, 7.0, 20),
                         labels = c("Target a (<= 6.5)", "Target b (6.6 - 7.0)", "Target c (>7.0)")),
           HbA1C_Ta = fct_relevel(HbA1C_T, levhba1c),
           HbA1C_Ta = ordered(HbA1C_Ta, levels = levhba1c),
           wt = `Physical Weight`,
           gender = Sex,
           ethnic = `Ethnic Group`,
           dmage = `Diabetes Age`,
    ...
    select(HbA1c, HbA1C_Ta, Age, gender, dmage, ht, wt, bmi, bmic, wc, tc, Dyslipidaemia) %>%

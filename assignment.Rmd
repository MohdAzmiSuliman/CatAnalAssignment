...
anads1 <- anads0 %>%
    mutate(HbA1C_T = cut(HbA1c, breaks = c(0, 6.5, 7.0, 20),
                         labels = c("Target a (<= 6.5)", "Target b (6.6 - 7.0)", "Target c (>7.0)")),
           HbA1C_Ta = fct_relevel(HbA1C_T, c("Target c (>7.0)", "Target b (6.6 - 7.0)", "Target a (<= 6.5)")),
           HbA1C_Ta = ordered(HbA1C_Ta, levels = c("Target c (>7.0)", "Target b (6.6 - 7.0)", "Target a (<= 6.5)")),
           wt = `Physical Weight`,
           gender = Sex,
           ethnic = `Ethnic Group`,
           dmage = `Diabetes Age`,
    ...
    mutate(ethniccol = fct_collapse(ethnic, malay = "Malay",
                 chinese = "Chinese",
                 indian = "Indian",
                 other = c("Bajau", "Murut", "Orang Asli (Peninsular)", "Iban", "Foreign National", "Others", "Unknown")),
    ...
    select(HbA1c, HbA1C_Ta, Age, gender, ethnic, ethniccol2, dmage, ht, wt, bmi, bmic, wc, tc, Dyslipidaemia) %>%

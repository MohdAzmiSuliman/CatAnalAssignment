...
anads2 <- anads1 %>% 
  mutate(tcc = cut(tc, breaks = c(0, 5.2, 6.2, 14),
         ...
         agec = cut(Age, breaks = c(0,18,50,60,70,100),
                    labels = c("< 18 yo", "19 - 50 yo", "51 - 60 yo", "61 - 70 yo", "above 80 yo")))
freq(anads2$agec)
freq(anads2$tcc)
```
...
```{r}
summary(clm_mod2R)
tidy(clm_mod2R, conf.int = T)
tidy(clm_mod2R, exponentiate = T, conf.int = T)
```


```{r}
result_clmmod2 <- matrix(c("Variables", "", "Coefficient", "SE", "p-value", "OR (95% CI)",
                           "Intercept 1 (A|B)*", "", -0.744, 0.109,"<0.001","",
                           "Intercept 2 (B|C)*", "", -0.293, 0.109, 0.007, "",
                           "BMI Category", "", "", "", "", "",
                           "", "Normal", 0, "", "", 1,
                           "", "Overweight", 0.146, 0.074, 0.048, "1.158 (1.001, 1.338)"),ncol=6,byrow=TRUE)
result_clmmod2
